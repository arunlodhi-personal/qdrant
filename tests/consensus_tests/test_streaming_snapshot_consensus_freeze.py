...
def test_streaming_snapshot_freezes_collection_via_consensus(tmp_path: pathlib.Path):
    ...
    assert "search" in frozen, (
        f"Expected search timeout but it completed. streaming={still_streaming}"
    )
    assert len(frozen) >= 2, f"Only search froze, expected systemic freeze: {frozen}"
    print(f"Test PASSED: {len(frozen)} operations frozen by consensus deadlock")
